...
export function buildSpotDetailPayload(spotId: number, viewerUserId: number | null) {
	...
	if (viewerUserId != null) {
		const existing = db
			.select()
			.from(votes)
			.where(and(eq(votes.userId, viewerUserId), eq(votes.spotId, spotId)))
			.get();
	...
	const childMicroSpots = hasMicro
		? db
				...
				.from(spots)
				.where(
					and(eq(spots.deleted, false), eq(spots.isMicro, true), eq(spots.parentSpotId, spot.id))
				)
				.all()
				.map((s) => ({
					id: s.id,
