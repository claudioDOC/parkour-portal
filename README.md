...
- **Read-only Status API (für KI/Automation/WhatsApp)**: Setze `PUBLIC_STATUS_API_KEY` (ein Key) oder `PUBLIC_STATUS_API_KEYS` (mehrere, komma-getrennt). Abruf über `GET /api/public/status/today` und `GET /api/public/status/next` mit Header `x-api-key: <key>` (alternativ `Authorization: Bearer <key>`). Antwort enthält kompakte Trainingsdaten inkl. Teilnehmer, Gäste, Abmeldungen, Spot-Votes und `summaryText` für Bots.
