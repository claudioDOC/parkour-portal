...
- **Strafrunde bei stillem Fernbleiben**: Trägt ein Admin jemanden ohne Grund als „nicht erschienen“ ein (Reguläres Training, Datum heute oder früher), läuft automatisch bis zum nächsten Training eine spielerische Strafe — Warnhinweis beim Öffnen von App und Portal (20 Sekunden nicht wegklickbar), **„Wer zieht“ nur als Fragezeichen** und beim nächsten Training ein **falscher Spot** (der nächstgelegene andere, auch im „Spot fix“-Push). Die Strafe endet mit dem Straf-Training von selbst; fällt es aus, wandert sie aufs nächste. Admin-Aktionen `no_show_penalty` (Nachtrag) und `remove_penalty` (Gnade); Tabelle `no_show_penalties`, Logik in `src/lib/server/noShowPenalty.ts`.
